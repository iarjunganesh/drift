...
| 2:00–2:20 | Architecture diagram | Follow feeds → Scout → Synthesizer → Insight → Briefing. Explain that the local one-shot capture path persists source evidence, generated Insights, embeddings, and audit data; scheduled population, reviewed real captures, and hosted verification remain the boundary. |
...
reproducible today, and the local capture path is implemented but not yet
presented as hosted live analysis. Every captured insight carries citations,
confidence, an audit label, and a bounded action.”
