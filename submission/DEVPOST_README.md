...
verified on 2026-07-19. The deployed app build is `v0.10.2`, verified live the
same day (`/health` reports `0.10.2` in `DRIFT_MODE=live`, `/docs`
returns `200`, Vercel-origin CORS allows `GET, POST`), so the hosted API now
runs the `v0.10.2` release with the corrected `upstream_release_type` values
(ADR-012).
...
| **Demo video** — 3-min narrated walkthrough (Codex + GPT-5.6) | [▶ Watch on YouTube](https://youtu.be/6sbIz0ZR8Hw) |
...
documentation follow-up are tied to fourteen project initiatives. The grounded
...
Eleven decisions currently constrain the implementation. They are intentionally
...
| **Current source release** | `v1.0.0` — the submission-final version bump: demo video published, submission docs synced, no capture/Insight/claim/provider path added. `backend/`, `frontend/`, and `integrations/` behavior is unchanged from `v0.10.2`, which added the deterministic `upstream_release_type` classification ([ADR-012](https://github.com/iarjunganesh/drift/blob/main/docs/adr/012-deterministic-version-classification.md)) applied to the three reviewed Insights with no self-declared source statement (JAX, NCCL, TensorRT → `minor`/`patch`/`minor`). Not yet verified as the hosted application — pending push and redeploy. The scrubbed MCP response archive remains pending |
...
app is the deployed `v0.10.2` build, verified live on 2026-07-19 (`/health`
reports `0.10.2`, `/docs` returns `200`, `/briefing` returns the five reviewed
Insights with the corrected `upstream_release_type` values, and Vercel CORS
allows `GET, POST`).
...
verified. Paid `/search` and `/chat` were not re-invoked for the `v0.10.2`
verification.
...
**Configure a client.** Point `DRIFT_API_URL` at a local fixture instance
...
> MCP is an additional consumption channel, not a repositioning: DRIFT stays
