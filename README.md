...
| **Current source release** | `v1.0.0` — the submission-final version bump: demo video published, submission docs synced, no capture/Insight/claim/provider path added. `backend/`, `frontend/`, and `integrations/` behavior is unchanged from `v0.10.2`, which added the deterministic `upstream_release_type` classification ([ADR-012](docs/adr/012-deterministic-version-classification.md)) applied to the three reviewed Insights with no self-declared source statement (JAX, NCCL, TensorRT → `minor`/`patch`/`minor`). Not yet verified as the hosted application — pending push and redeploy. The scrubbed MCP response archive remains pending |
