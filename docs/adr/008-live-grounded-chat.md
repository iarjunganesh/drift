...
The source patch is tagged on `feature/v0.9.0-final-evidence` and shipped in
the `v0.9.x` line as `v0.9.1` (hosted `/health` verified `0.9.1` on
2026-07-18, before the `v0.10.0` redeploy later that day); the grounded `/chat`
behavior itself was not re-invoked in that check.
