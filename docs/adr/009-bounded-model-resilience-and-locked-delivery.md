...
Live model calls use an application-owned retry budget, per-attempt timeout,
queue-bounded concurrency bulkhead, circuit breaker, and conservative spend
settlement. Python dependencies have one resolution authority: `uv.lock`, used
unchanged by local development, CI, release builds, and the Railway image.
